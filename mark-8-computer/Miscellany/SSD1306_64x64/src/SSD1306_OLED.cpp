...
// #endif
	...
	EndTransfer();
	needsRepaint = 1;
	for (uint8_t i = 0;i < 8;i++) dirtyColumns[i] = 0xFF;
	for (uint16_t i = 0;i < 64 * 8;i++) columnPixels[i] = 0x00;
}

void SSD1306Simple::loadColumn(uint8_t x,uint8_t y,uint8_t *data,uint8_t count) {
    configureBus();
    StartTransfer();
    SelectCommand();
...
void SSD1306Simple::setPixel(uint8_t x,uint8_t y,uint8_t isOn) {
	if (((x|y) & 0xC0) != 0) return;
	uint8_t bit = (0x01 << (y & 7));
	uint16_t offset = x * 8 + (y >> 3);
	if (((columnPixels[offset] & bit) != 0) != (isOn != 0)) {
		if (isOn) {
			columnPixels[offset] |= bit;
		} else {
			columnPixels[offset] &= (bit ^ 0xFF);
		}
		needsRepaint = 1;
		dirtyColumns[(x >> 8)] |= (0x01 << (x & 7));
	}
}

void SSD1306Simple::repaint() {
	if (needsRepaint == 0) return;
	needsRepaint = 0;
	for (uint8_t x = 0;x < 8;x++) {
		uint8_t dBits = dirtyColumns[x];
		if (dBits != 0) {
			for (uint8_t b = 0;b < 8;b++) {
				if ((dBits & (0x01 << b)) != 0) {
					uint8_t xc = x * 8 + b;
					loadColumn(xc*2,0,columnPixels+xc*8,8);
					loadColumn(xc*2+1,0,columnPixels+xc*8,8);
				}
			dirtyColumns[x] = 0;
			}
		}
