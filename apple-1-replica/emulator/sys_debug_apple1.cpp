...
void DBGXRender(int *address,int isRunMode) {
	...
	if (isRunMode != 0) {
		SDL_Rect rc;
		int cSize = 3;																// Character pixel size.
		int w = TRMCommand(TRC_GETWIDTH,0,0,0);
		int h = TRMCommand(TRC_GETHEIGHT,0,0,0);
		rc.w = cSize * w * 6;
		rc.h = cSize * h * 8;
		rc.x = WIN_WIDTH/2 - rc.w/2;
		rc.y = WIN_HEIGHT - 64 - rc.h;
		SDL_Rect rc2 = rc;
		rc2.w += 20;rc2.h += 20;rc2.x -= 10;rc2.y -= 10;
		GFXRectangle(&rc2,0xFFF);
		rc2.w -= 4;rc2.h -= 4;rc2.x += 2;rc2.y += 2;
		GFXRectangle(&rc2,0x000);
		for (int x = 0;x < w;x++) {
			for (int y = 0;y < h;y++) {
				int c = TRMRead(x,y);
				if (c > ' ')
					GFXCharacter(rc.x+x*6*cSize,rc.y+y*8*cSize,c,cSize,0x0F0,-1);
			}
		}
	}
}	