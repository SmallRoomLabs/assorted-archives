...
void SSD1306Simple::loadColumn(uint8_t x,uint8_t y,uint8_t d1,uint8_t d2,uint8_t d3) {
    configureBus();
    StartTransfer();
    SelectCommand();
...
static const PROGMEM uint16_t smallFont[] = {
	#include "__smallfont.h"
};

#define SH(c) 	((c) << 1)

void SSD1306Simple::write(uint8_t x,uint8_t y,uint8_t ch) {
	uint16_t w = pgm_read_word(smallFont + (ch & 127));
	loadColumn(x*4,y,SH(w & 0x1F),SH((w >> 5) & 0x1F),SH((w >> 10) & 0x1F));	
}
