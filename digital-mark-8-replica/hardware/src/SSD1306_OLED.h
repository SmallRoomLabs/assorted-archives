...
#if I2C
#if RESET_PULSE
	SSD1306Simple(int8_t rst) : rst_(rst) {}
#else
	SSD1306Simple() {}
#endif
#else
#if HW_SPI
	SSD1306Simple(int8_t dc, int8_t rst, int8_t cs)
		:dc_(dc), rst_(rst), cs_(cs) {}
#else // software SPI
	SSD1306Simple(int8_t data, int8_t clk, int8_t dc, int8_t rst, int8_t cs)
		:data_(data), clk_(clk), dc_(dc), rst_(rst), cs_(cs) {}
#endif
#endif

	void init();
	void write(uint8_t x,uint8_t y,uint8_t ch);
private:
	void loadColumn(uint8_t x,uint8_t y,uint8_t d1,uint8_t d2,uint8_t d3);
	void StartTransfer();
	void SelectCommand();
	void SelectData();
...
#endif
	uint8_t dirtyColumns[8];
	uint8_t needsRepaint;
	uint8_t columnPixels[64*8];
};
