	...
	for (int col = 0;col < TWIDTH;col++)
		for (int row = 0;row < THEIGHT;row++) {
			...
			if ((ch & 0x80) != 0 && GFXTimer()/300%2 == 0) {
				GFXCharacter(GRID(col+16,row+3),ch & 0x7F,GRIDSIZE,0x000,0xF80);
			} else {
				GFXCharacter(GRID(col+16,row+3),ch & 0x7F,GRIDSIZE,0xF80,0x000);
			}
		}
}	
...
void DBGXWriteTTYDisplay(BYTE8 col,BYTE8 row,BYTE8 character) {
	if (col >= 0 && col < TWIDTH && row >= 0 && row <= THEIGHT) {
		ttyBuffer[col+row*TWIDTH] = character;
	}
}
