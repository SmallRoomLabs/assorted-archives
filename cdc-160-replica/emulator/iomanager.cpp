...
BYTE8 DVCTypewriter(BYTE8 deviceID,char fn,WORD16 *pData) {
	...
	switch(fn) {
		...
		case 'Q':	if (typeWriterStatus.isDataAvailable == 0) {					// No data waiting ?
						BYTE8 count = FLXRead(typeWriterStatus.data,				// Try to read some in.
														sizeof(typeWriterStatus.data));
						if (count > 0) {											// Data read ?
							typeWriterStatus.isDataAvailable = 1;					// Data is now available.
							typeWriterStatus.dataPointer = 0;						// Reset pointer.
							typeWriterStatus.data[count] = '\0';					// Add terminating zero.
						}
					}
					retVal = (typeWriterStatus.isDataAvailable == 0);				// Is data available ? No return error.
					break;														
