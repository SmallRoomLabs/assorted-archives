...
class TeletypeMapper(Mapper):																		# 5 bit teletype (note use of figure shift)
	def __init__(self):
		...
		9|103|0|115|1|135|4|112|5|101|7|134|;|117|2|131|/|127|6|125|"|121|_|04
		"""
		Mapper.__init__(self,mapper)
	def getShift(self):
		return 0o33
...
class CDC160Assembler:
	...
	def dumpBiOctal(self):
		...
		for address in range(low,high+1):													
			...
			paperTape.write(self.tapeEncode(((data >> 6) << 1) | 1))								# upper half with seventh hole
			paperTape.write(self.tapeEncode((data & 0o77) << 1))									# lower half without seventh hole.

		paperTape.write(self.tapeEncode(0o77 << 1))													# write a word without seventh hole
																									# terminates the read.
		paperTape.close()
