...
//------------------------------------------------------------------------------
class SSD1306ElfDisplay {

public:
#if I2C
#if RESET_PULSE
	SSD1306ElfDisplay(int8_t rst) : rst_(rst) {}
#else
	SSD1306ElfDisplay() {}
#endif
#else
#if HW_SPI
	SSD1306ElfDisplay(int8_t dc, int8_t rst, int8_t cs)
		:dc_(dc), rst_(rst), cs_(cs) {}
#else // software SPI
	SSD1306ElfDisplay(int8_t data, int8_t clk, int8_t dc, int8_t rst, int8_t cs)
	...
	void init();
	void clear();
	void refresh(uint8_t *screen);
	void displayBinary(uint8_t n);
private:
	void StartTransfer();
	void SelectCommand();
	void SelectData();
	...
	void drawHBar(uint8_t x,uint8_t y,uint8_t show);
	void drawVBar(uint8_t x,uint8_t y,uint8_t show);
    void sendCommand(uint8_t c);
    void sendData(uint8_t c);
    void writeBlock(uint8_t x,uint8_t y,uint8_t *b,uint8_t size);
    uint8_t screenBuffer[256];
