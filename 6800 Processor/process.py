...
modes["dr"] = { "eac":"FETCH();MA = MB","cycles":-1,"desc":"@1" }
modes["ex"] = { "eac":"FETCHWORD();MA = temp16;","cycles":0,"desc":"@1@2" }
modes["in"] = { "eac":"FETCH();MA = MB + IX;","cycles":0,"desc":"@1,X" }
...
for l in src:
	...
	if len(m.group(3)) == 2:
		...
		for mode in opcodes.group(1).split(","):
			...
			if mode == "im":
				body = body.replace("READ()","FETCH()")
			mnemonic = m.group(1)+" "+modes[mode]["desc"]
...
open("__6800mnemonics.h","w").write(",".join(m)+"\n")
