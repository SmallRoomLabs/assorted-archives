...
def process(s,opcode):
	...
	s = s.replace("%D","ABCDEHLM"[opcode/8 % 8])
	s = s.replace("%P","{0:02X}".format(opcode / 2 % 32))
	s = s.replace("%R","{0:02X}".format(opcode & 0o070))
	s = s.replace("%MAHL","MA = (H << 8)|L")
	s = s.replace("%C",["NC","NZ","P","PO","C","Z","M","PE"][opcode / 8 % 8])
	s = s.replace("%T",["carry == 0","pszValue != 0","(pszValue & 0x80) == 0","_IsParityEven(pszValue) == 0","carry != 0","pszValue == 0","(pszValue & 0x80) != 0","_IsParityEven(pszValue) != 0"][opcode / 8 % 8])
	return s
