...
#define WIN_TITLE 		"Mike 4 Emulator"											// Initial Window stuff
...
#define DEBUG_RAMSTART 		(0x2000)												// Initial RAM address for debugger.
#define DEBUG_SHIFT(d,v)	((((d) << 4) | v) & 0xFFFF)								// Shifting into displayed address.

#define DEBUG_KEYMAP(k,r) 	DRVProcessDebugKey(k,r)									// Runtime can remap/process keys etc.

void DBGXRender(int *address,int showCPU);											// Render the debugger screen.
void DBGXSetScopewriter(char *text);												// Set scopewriter text
