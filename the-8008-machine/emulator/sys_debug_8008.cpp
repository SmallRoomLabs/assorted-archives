...
void DBGXRender(int *address,int showDisplay) {
	...
	SDL_Rect rc;rc.w = xSize * 128;rc.h = ySize * 64;
	...
	SDL_Rect rcp;rcp.w = xSize-1;rcp.h = ySize-1;
	for (int y = 0;y < 64;y++) {
		rcp.y = y * ySize + rc.y;
		for (int x = 0;x < 16;x++) {
			BYTE8 b = HWIGetVideoMemory()[x+y*16];
			if (b != 0) {
				rcp.x = x * 8 * xSize + rc.x;
				while (b != 0) {
					if ((b & 0x80) != 0) GFXRectangle(&rcp,0xF80);
					rcp.x += xSize;
					b = (b << 1) & 0xFF;
				}
			}
