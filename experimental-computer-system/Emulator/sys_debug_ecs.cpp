...
void DBGXRender(int *address,int showDisplay) {
	...
	const char *labels2[] = { "LD","RD","PC","ST",NULL };
	n = 0;
	...
	GFXNumber(GRID(28,0),IFRead(IFW_LEFTDISPLAY),2,8,GRIDSIZE,DBGC_DATA,-1);
	GFXNumber(GRID(28,1),IFRead(IFW_RIGHTDISPLAY),2,8,GRIDSIZE,DBGC_DATA,-1);
	n = 2;																			// PCTR
	...
	SDL_Rect rc,rc2;rc.w = WIN_WIDTH * 8 / 10;rc.h = WIN_HEIGHT / 8;
	rc.x = (WIN_WIDTH-rc.w)/2;rc.y = WIN_HEIGHT * 3 / 4;
	...
	for (int b = 0;b < 17;b++) {
		if (b != 8) {
			rc2.w = rc.w / 17;rc2.h = rc.h;rc2.x = rc.x + b * rc2.w;rc2.y = rc.y;
			rc2.w -= border * 2;rc2.h -= border * 2;rc2.x += border;rc2.y += border;
			GFXRectangle(&rc2,0xFFF);
			rc2.w -= 2;rc2.h -= 2;rc2.x++;rc2.y++;
			on = (b < 8) ? (IFRead(IFW_LEFTDISPLAY) & (0x80 >> b)) : (IFRead(IFW_RIGHTDISPLAY) & (0x80 >> (b - 9)));
