...
#ifdef ARDUINO																		// Arduino does not have 16k RAM (+ boot)
#define READ() 		MB = ((MA < RAMSIZE) ? ramMemory[MA] : DEFAULT_BUS_VALUE);
#define WRITE() 	if (MA < RAMSIZE) ramMemory[MA] = MB;
#else
#define READ() 		MB = ramMemory[MA]												// Other stuff does.
#define WRITE()		ramMemory[MA] = MB 												// So we just do it into the memory array.
#endif
...
void CPUReset(void) {
	...
	PCTR = 0x3F00;																	// Boot ROM
	ramMemory[PCTR] = 0x44;															// As a short term fudge, we have JMP $0000
	ramMemory[PCTR+1] = 0x00;
	ramMemory[PCTR+2] = 0x00;
}
