...
;         (e.g. Spaces routine in El Dumpo)
;
...
;
;   These routines (currently 254 bytes) should ideally be on a page on their own, alternatively the Character
...
CDControl_Port = 8
CDControl_SetY = 100o
CDControl_ScreenOff = 201o
CDControl_Set = 202o
CDControl_Clear = 203o
CDControl_SetAdv = 204o
CDControl_ClearAdv = 205o
CDControl_ScreenOn = 207o
...
PrintCharacter:
    ...
    cpi     32                                                  ; don't print control characters.
    jc      PCReloadRegisters
PCYPositionLoader:
    mvi     e,0                                                 ; fetch the Y coordinate into E
...
PCXPositionLoader:
    ...
    ori     CDControl_SetY                                      ; make into a SET-Y command
    out     CDControl_Port
...
PCNewLineCode:
    ...
    cpi     60                                                  ; have we space for another line ?
    jc      _PCNoHomeRequired
    xra     a                                                   ; home to line 0
_PCNoHomeRequired:
    mov     m,a                                                 ; write it back.
    ori     CDControl_SetY                                      ; make it a display:setY instruction
    out     CDControl_Port
    mvi     e,6*64/2                                            ; number of writes to do to clear the next line.
    mvi     a,CDControl_ClearAdv
PCNewLineClear:
    out     CDControl_Port                                      ; doing it in twos is quicker and also use byte counter
    out     CDControl_Port
    dcr     e
    jnz     PCNewLineClear  
...
NextBit:
    ...
    mvi     a,0                                                 ; clear A
    sbi     (0-CDControl_ClearAdv) & 255                          ; make A into 204/205 using borrow because it's backwards.
    out     CDControl_Port
    ret
